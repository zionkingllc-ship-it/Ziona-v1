...
import { useDiscoverFeed, useDiscoverSearch } from "@/hooks/useDiscover";
...
export default function DiscoverCategoryScreen() {
  const { categoryId, label, slug } = useLocalSearchParams<{
    categoryId: string;
    label?: string;
    slug?: string;
  }>();
  ...
  } = useDiscoverSearch(debouncedSearch, slug ?? undefined, debouncedSearch.trim().length > 0);
  ...
  if (isError && posts.length === 0) {
    const feedback = getNetworkModalCopy(
      error,
      "We couldn't load posts right now. Please try again.",
    );
    return (
      <SafeAreaView style={{ flex: 1, backgroundColor: colors.white }}>
        <View style={{ flex: 1 }}>
